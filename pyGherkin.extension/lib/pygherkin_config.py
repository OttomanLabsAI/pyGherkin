...
# The branch Install Update pulls from. The extension tracks this branch's
# tip (rolling), not tagged releases. Override with the 'github_branch' key.
...
def get_github_branch():
    """Branch Install Update pulls from ('github_branch' settings key,
    default DEFAULT_GITHUB_BRANCH = 'main')."""
    s = load_settings()
    return (s.get("github_branch") or DEFAULT_GITHUB_BRANCH).strip()
