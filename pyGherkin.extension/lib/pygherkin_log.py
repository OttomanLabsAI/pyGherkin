# -*- coding: utf-8 -*-
"""Shared logging - writes to pyRevit output *and* a timestamped file.

Ported from pyMEP so the Install Update button reports the same way.
"""

import os
import datetime


class Logger(object):
    """Wrap a pyRevit output object and an optional log file.

    Usage:
        log = Logger(output, "InstallUpdate")
        log("### Starting")
        ...
        log.close()
    """

    def __init__(self, output, name):
        self._output = output
        log_dir = os.path.join(os.environ.get("APPDATA", ""), "pyRevit", "Logs")
        if not os.path.exists(log_dir):
            try:
                os.makedirs(log_dir)
            except Exception:
                pass
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        self.log_path = os.path.join(log_dir, "{}_{}.log".format(name, ts))
        try:
            self._lf = open(self.log_path, "w")
            self._lf.write("{}  {}\n{}\n\n".format(
                name, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "=" * 60))
            self._lf.flush()
        except Exception:
            self._lf = None

    def __call__(self, msg):
        if self._output is not None:
            try:
                self._output.print_md(msg)
            except Exception:
                pass
        # A logged traceback pins the window open regardless of the
        # auto-close setting - error reports must stay readable.
        try:
            if "Traceback (most recent call last)" in msg:
                self.keep_open = True
        except Exception:
            pass
        if self._lf is not None:
            safe = ""
            for ch in msg.replace("**", "").replace("###", "").replace("####", "") \
                         .replace(u"→", "->").replace(u"✓", "[OK]"):
                safe += ch if ord(ch) < 128 else "?"
            try:
                self._lf.write(safe + "\n")
                self._lf.flush()
            except Exception:
                pass

    def close(self):
        if self._lf is not None:
            try:
                self._lf.close()
            except Exception:
                pass
            self._lf = None
        # Optionally close the command's output window too (Settings key
        # 'auto_close_output'). Never when an error/traceback was logged.
        try:
            if not getattr(self, "keep_open", False) and \
                    self._output is not None:
                from pygherkin_config import get_auto_close_output
                if get_auto_close_output():
                    self._output.close()
        except Exception:
            pass
